...
{
    // keep track if the edge is removed
    ...
    if (adj_list_out[v1] -> vertex == v2)
    {
        delptr = adj_list_out[v1];
        adj_list_out[v1] = adj_list_out[v1] -> next;
        edgeRemoved = true;
        delete delptr;
    }
    else {
        Node* currPtr = adj_list_out[v1];
        while (currPtr -> next != nullptr)
        {
            if (currPtr -> next -> vertex == v2)
            {
                delptr = currPtr -> next;
                // assign the last Node's next to the deleted Node's next
    ...
    // check if the first Node is the one to be deleted
    if (adj_list_in[v2] -> vertex == v1)
    {
        delptr = adj_list_in[v2];
        adj_list_in[v2] = adj_list_in[v2] -> next;
        delete delptr;
    }
    else {
        Node* currPtr = adj_list_in[v2];
        while (currPtr -> next != nullptr)
        {
            if (currPtr -> next -> vertex == v1)
            {
                delptr = currPtr -> next;
                // assign the last Node's next to the deleted Node's next
                currPtr -> next = currPtr -> next -> next;
